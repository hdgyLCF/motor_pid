...
一个基于 STM32F103C8T6 的完整 PID 电机串级控制解决方案，采用 PWM 控制、编码器反馈测速、电流反馈和双环 PID 算法（速度外环 + 电流内环）。

##  项目特性

- **串级 PID 控制**：双环控制结构，速度外环 + 电流内环，增量式 PID 算法，支持积分饱和保护、测量低通滤波、输出斜率限制
- **PWM 输出**：1kHz PWM 频率，PA8 端口驱动电机（TIM1）
- **编码器反馈**：20 脉冲/转的编码器，通过 TIM4 编码器模式测速
- **电流反馈**：ADC1 通道1 测量电机电流
- **定时控制**：TIM2 提供 10ms 周期的 PID 控制任务调度
- **串口通信**：UART1 (115200) 用于 printf 调试和命令交互
- **死区补偿**：处理电机低转速死区，确保控制稳定性
- **实时监测**：通过串口重定向 输出目标转速、实际转速、目标电流和实际电流，便于 VOFA 等工具可视化
...
- **编码器**：编码器电机附带
- **电流传感器**：连接到 ADC1 通道1 (PA1)，用于测量电机电流
- **其他**：USB 转串口模块（调试用）
